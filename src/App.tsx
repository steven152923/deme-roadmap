...
import type { ActivityEntry, RoadmapCard, RoadmapData, RoadmapRelease, Stage, ViewId, WorkKind } from './types';
...
function appendActivity(entries: ActivityEntry[], entry: ActivityEntry) { return [...entries.slice(-249), entry]; }
...
export default function App() {
  ...
  function createQuick(value: QuickCaptureValue) {
    const id = createItem(value.stage, false, {
      title: value.title, area: value.area, priority: value.priority, kind: value.kind, bugSeverity: value.bugSeverity,
      releaseId: value.releaseId, targetDate: value.targetDate, today: value.today,
    });
  ...
  function promoteInboxThought(id: string) {
    setData((current) => {
      ...
      const text = note.text.trim(); const title = text.length > 88 ? `${text.slice(0, 85).trim()}…` : text;
      const card = { ...newCard('planned', current.settings.areas[0] ?? 'Core', maxOrder(current.cards, 'planned'), 'feature'), title, description: text.length > 88 ? text : '' };
      window.setTimeout(() => setSelectedId(card.id), 0);
      return { ...current, inbox: current.inbox.filter((item) => item.id !== id), cards: [...current.cards, card], activity: appendActivity(current.activity, activity('created', `Promoted “${title}” from the Idea Inbox`, card.id)) };
    });
  ...
  function moveCard(id: string, stage: Stage) {
    setData((current) => {
      ...
      const moved = { ...card, stage, ...(stage === 'ideas' ? { kind: 'bug' as const } : {}), sortOrder: order, updatedAt: new Date().toISOString() };
  ...
  function reorderCard(sourceId: string, targetId: string) {
    setData((current) => {
      ...
      return {
        ...current,
        cards: current.cards.map((card) => orderMap.has(card.id) ? { ...card, stage: target.stage, ...(target.stage === 'ideas' && card.id === source.id ? { kind: 'bug' as const } : {}), sortOrder: orderMap.get(card.id)!, updatedAt: card.id === source.id ? new Date().toISOString() : card.updatedAt } : card),
  ...
  function deleteRelease(id: string) {
    ...
    setData((current) => ({ ...current, releases: current.releases.filter((item) => item.id !== id), cards: current.cards.map((card) => card.releaseId === id ? { ...card, releaseId: '', updatedAt: new Date().toISOString() } : card), activity: appendActivity(current.activity, activity('release', `Deleted release “${release.name}”`)) }));
  ...
  function changeSettings(patch: Partial<RoadmapData['settings']>) {
    setData((current) => { const settings = { ...current.settings, ...patch }; let cards = current.cards; if (patch.areas && patch.areas.length) { const fallback = patch.areas[0]; cards = cards.map((card) => patch.areas!.includes(card.area) ? card : { ...card, area: fallback, updatedAt: new Date().toISOString() }); } return { ...current, settings, cards }; });
  }
  ...
  async function importBackup() { if (!window.demeRoadmap) return; try { const result = await window.demeRoadmap.importBackup(); if (!result.canceled && result.data) { const next = normaliseRoadmap(result.data); setData(next); setView(next.settings.startView === 'releases' ? 'board' : next.settings.startView); setBoardReleaseId(next.releases.find((release) => release.status === 'active')?.id ?? next.releases.find((release) => release.status === 'planned')?.id ?? UNASSIGNED); setSelectedId(null); setReleaseEditorId(null); setToast('Backup restored'); } } catch { setToast('That backup could not be opened'); } }
  ...
  return (
    <div className="app-shell v2-shell v3-shell v4-shell">
      ...
      <aside className="sidebar v2-sidebar v3-sidebar">
        ...
        {activeRelease && <button className="sidebar-release" type="button" onClick={() => { setBoardReleaseId(activeRelease.id); navigate('board'); }}><span className="live-dot" /><span><small>Growing now</small><strong>{activeRelease.name}</strong></span></button>}
        ...
        <div className={`save-state ${saveState}`}><span /> {saveState === 'saving' ? 'Saving locally…' : saveState === 'error' ? 'Couldn’t save' : 'Safe on this PC'}</div><div className="version-mark">v0.4 ♡</div>
      </aside>
...
function DemeBrand() { return <div className="deme-brand-v2 deme-brand-v3"><div className="deme-d-mark"><span>D</span><i /></div><div className="deme-brand-copy"><strong>Deme</strong><small>roadmap diary ✦</small></div><Sparkles className="brand-sparkle" size={18} /></div>; }
